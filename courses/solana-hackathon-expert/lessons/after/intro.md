...
Close the plan. This is the course's concept map, the whole of it. Every file sits in one repo, dated, quoting the page it came from, which is why the second season costs a fraction of the first: the season page gets re-pulled, the pitch gets rewritten, and the rest stays.

```text
four questions under every rubric:  is the problem real / does it work / does the story land / is this the team
Colosseum adds two:                 Viability, Traction
house rules:                        run before naming / pitch from day 0 / score every rung against the page

rungs:  1 prize table                  9 devnet slice, one real transaction
...
pitch thread:  v0 (rung 3) -> v1 (rung 6) -> v2 (rung 10) -> final (rung 12), four feedback rounds
carried:       the whole repo    re-pulled: the season page    rewritten: the pitch
```

## Checkpoint

Close the repo. Then, no notes, out loud: say the four questions under every rubric. Say what a hackathon win guarantees at the accelerator and what it does not. Say the rule Eternal puts on a prior hackathon product. Then say the two things from this whole course that decide most outcomes, in your own words.

If your two-item recall is 'the video they open first' and 'one product per person', you kept the two things that decide most outcomes. Everything else in the course is how to make those two go well.
