...
3. **Change one word in the command** and pull the season before Frontier:
   ...
   ```bash
   curl -sL https://superteam.fun/api/hackathon/cypherpunk | python3 -m json.tool
   ```
...
**A rehearsal** is any other competition you enter with the same package: a Colosseum global season is the target, and a Superteam Brasil seasonal hackathon before the next Colosseum season or a Superteam Earn side track alongside the season you enter *is a rehearsal*.
...
- **Building for the side track** and letting the main competition go: the regional row is the shortest path to a prize, and *what Colosseum's judges score is what makes a project worth a prize at all*, so hold both.
