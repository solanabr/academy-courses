...
Every season, teams with a working product lose on a form. A deadline converted in the wrong direction. A second submission from a teammate that breaks the one-per-person rule. A demo video that never made it into its field. A private repo nobody gave the reviewer access to. None of that is in the rubric. All of it is in the rules, on the same page as the rubric, a few paragraphs further down where fewer people read. The person that opens your submission first is not scoring Founder + Market Fit, they are checking that the video is there, that the repo opens, that the team has names on it. Completeness is the one line on this whole page a team controls fully. So today you read the rules the way you read the rubric in lesson 2, as a spec, and you submit the way you would ship a release, against a checklist written before the work starts.

## Do this

1. Assemble the package before you open any portal. Make a file called submission-package.md at the root of the toolkit repo and put one line per rung in it, with the path of the artifact and its date, from memory first and then corrected against the repo:

```text
submission-package.md, opened 2026-10-05
surface brief (rubric, deadline line, disqualifiers)            path, date
...
If a line has no path, the rung is missing, and today is the day you finish that rung and not submission day. Everything below assumes every line is filled.

2. Answer the Colosseum portal, all nine fields, from the file and in the page's order. Colosseum's portal, as of the 2026 World's Fair season, asks for nine things, and the page that lists them is colosseum.com/hackathon, the same page your surface brief quotes. Copy them into the package file in the page's words before you answer any of them:
...
![Each of the nine Colosseum portal fields is answered by a named course rung, from the pitch final for the name to the GTM one-pager and partner list for the last box.](assets/v01-table.webp)

3. Convert the deadline with a tool, never in your head, and write both results in the package file next to each other. The string to practice on is real. An Earn hackathon bounty listing read on 2026-09-06 carried the deadline 2026-09-08T02:59:59Z. Paste it into this:
...
On a laptop set to Brasília time the two lines print as 2026-09-08 02:59:59+00:00 and 2026-09-07 23:59:59-03:00. The listing says the eighth. Your calendar says the seventh, one second before midnight. A team that put "the 8th" in a group chat and planned to submit on the morning of the 8th has already lost, with a working product and a good video. The +00:00 in the command is the trailing Z written out, and on Python 3.11 or newer the Z works on its own, so on an older machine keep the long form. For Colosseum the string comes from your surface brief. Lesson 2 copied the season's end date from the live page, and as of 2026-09-06 the hour was the part the page had not printed, so the brief holds a slot for it. In the final week, re-read colosseum.com/hackathon, paste the hour the page prints into the command, and write the local result on the deadline line beside the UTC original. Then convert a second time with a different tool, a phone's world clock is enough, and only when both agree does the line get the word "checked" after it. Write the leader's name on that same line, since the page says the team leader must complete the submission before the deadline. My own rule, a preference and not a page rule, is to set the team's deadline a full day before the printed one, so the last day is for the screenshot and not for the form.

![The deadline is copied in UTC, converted twice, moved a day earlier for the team, re-checked the day before, and then the leader submits and the screenshot goes into the log.](assets/v02-comparison.webp)
...
5. Submit, in a fixed order. Open the live page one last time and compare its field list with the one in your file, since a portal that adds a field between the day you copied it and submission day is not a thing this lesson can rule out, and a field you never saw is a field left empty. Then the leader logs into the portal, pastes each answer from the file, reads each field back against the file before moving to the next one, submits, and screenshots the confirmation into the narrative log with the date. Off-season, when no portal is open, the mock form is this same file: the fields in the page's order, nothing blank, and the leader's name and the date on the last line in place of the screenshot.

![On submission day the leader diffs the live field list, pastes and reads back each field from the file, submits, logs the screenshot, and only then opens a rehearsal form.](assets/v03-timeline.webp)

6. Run the same package through a rehearsal surface if one is open, after or with the Colosseum form and never instead of it. Two places can receive it. An Earn side track is the same product entered on a Superteam Earn track attached to the same season, through Earn's own form, with custom eligibility questions and a deadline in UTC, and the listing you practiced the conversion on is the shape of it: an Earn hackathon bounty read on 2026-09-06, Hackathon Universitaria Superteam Brasil, US$3,000 USDG paid as 1500, 1000 and 500 by rank, region Brazil, deadline 2026-09-08T02:59:59Z, and an eligibility form that asked for the institution and the course. A Superteam Brasil edition receives the package on its own platform, hackathon.superteam.com.br, in a window that sits before a Colosseum season rather than inside one, and the flow is the same across its editions: register, build with the WhatsApp community, submit a deck plus a demo video plus a repo, finalists pitch to a banca, and the prize arrives through Superteam Earn by the Earn-to-Pix guide. Passo Fundo 2026, read on 2026-09-06, had the leader submit deck, video and repo on the platform by September 9 at 12:00 as the page prints it, Brasilia time (UTC-3), with payment through Earn. The Vibeathon, read the same day, required registering on both the platform and Earn with the same e-mail and submitting on the Earn listing. Editions and dates are read from the live page each time, and the ones cited here are dated examples. So the method is one sentence: open its page, copy its deliverables and its deadline into the package file with the date, answer its questions, and paste the rest. Do not rebuild the package for it and do not write a second description. Before any second submission, read the rule on the surface's own page and the one-per-individual rule on the Colosseum page, on the day, and write in the package file which rule you read and where. If the two rules cannot both be kept, skip the surface and write down why.

![Before a second submission the team reads the surface's rule and Colosseum's one-per-individual rule, submits only if both hold, and otherwise skips the surface.](assets/v04-diagram.webp)

## Done when

- No portal field is empty, and every answer in submission-package.md points at a rung.
...
- Entering a rehearsal surface by reflex, which happens when the rules live in a chat and not in the file. The Vibeathon's same e-mail rule existed only on the rehearsal page, in Portuguese.
- The tradeoff: a rehearsal surface costs a second form and can collide with the one-product-per-individual rule if the surface is itself a Colosseum track, so skip a surface rather than break a rule.

## Checkpoint

Close the package file. Then, no notes, out loud: say the nine portal fields in any order, the four lines on the disqualifier checklist, and both halves of the deadline line. Then say which field a judge opens first, and which rule would have disqualified your team if nobody had read it. If you can name the field a judge opens first and the rule that would have disqualified you, the capstone is done. If either would not come out, open the page and read the part under the fields. It is short.

## Next

The capstone is done when the confirmation is in the log, the last artifact the course's promise asked for. Next lesson's first action is a file called next-season-plan.md next to this package, with two sentences copied from the live page about what happens once submissions close, the 15-minute interview a smaller group is invited to and the announcement roughly a month after the deadline. From there it walks why a lost pitch is not a lost project. You are ending the month with a package a judge can open, a form nobody composed at the portal and a screenshot with a date on it. Keep the screenshot, because a month from now, whatever the winners page says, it is the proof that the form did not beat you.
