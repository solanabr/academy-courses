...
![The Token-2022 transfer fee formula, ceiling division capped at maximumFee, shown as annotated TypeScript with the 1,001-units-withholds-11 rounding example called out.](assets/v01-annotated-code.webp)
...
![Table of the four authority seats, their powers, and revocation outcomes, from a frozen fee schedule to permanently stranded withheld fees.](assets/v02-table.webp)
...
![The authority signs set_transfer_fee mid-epoch, the 200 bps schedule arms at a future epoch boundary, and transfers keep charging 100 bps until it passes.](assets/v03-timeline.webp)
...
![Diagram of a 250 SPROUT sale at a 1% fee: the buyer's account holds 247.5 spendable plus 2.5 withheld in its TransferFeeAmount slot, while the treasury sits empty and uninvolved.](assets/v04-diagram.webp)
...
![Flowchart of the fee lifecycle: fees withheld on recipient accounts, permissionless harvest to the mint, authority-gated withdraw to a treasury, plus a direct authority-gated accounts-to-destination route.](assets/v05-flowchart.webp)
...
![A flat raw-amount line diverges from a rising displayed-amount curve on a 5% interest-bearing mint, with amount_to_ui_amount as the only sanctioned bridge between them.](assets/v06-diagram.webp)
...
![Comparison of InterestBearingConfig's continuous display drift and ScaledUiAmount's stepwise jumps, with neither minting supply and rule 4 allowing a mint to carry only one.](assets/v07-comparison.webp)
...
![Bar chart of measured compute units for the five lab transactions in lifecycle order, from mint creation through fee transfers and harvest to the treasury withdraw.](assets/v08-chart.webp)
