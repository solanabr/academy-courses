...
The autonomy fade for this lesson: the reading of the source rules is worked in full, I walk every line with you. The validator is a completion exercise: rule 4 ships already implemented, rules 1, 2, 3, and 5 are yours to port. And the module's single coding challenge, at the end, is solo: the full validator proven by tests, no scaffolding.
...
![Flowchart with one gate on extension-only behavior, classic SPL as the no-branch, and a yes-branch that drafts an extension set, validates it against the five source rules, then initializes permanently.](assets/v01-flowchart.webp)
...
![A comparison showing the same docs page omitting PermissionedBurn one day and listing it the next, beside the commit-pinned source enum whose 29 production variants stay as recorded.](assets/v02-comparison.webp)
...
![Table of the five conflict rules with the extensions involved, each constraint, and its class (bridge, additive, one-way dependency, mutual exclusion, conditional), cited to the pinned source lines.](assets/v03-table.webp)
...
![Table running three product briefs through the framework, showing requirements translated to extension sets, the rule judging each draft, and the additive fixes that made two rows legal.](assets/v04-table.webp)
...
![Stacked bar splitting the 29 production extension variants into 20 unconstrained, 5 in the conflict guards only, 2 in the forced-pair lookup only, and 2 in both.](assets/v05-chart.webp)
...
![Three gates stand between an extension set and a tradeable token: code-enforced init legality, which check-combo covers, then unenforced doc cautions, then per-venue acceptance policy.](assets/v07-diagram.webp)
...
![Side-by-side of the Rust rule-4 guard and its TypeScript port, annotated to show the flag-to-has() mapping and that the validator adds per-rule reasons where the program has one collapsed error.](assets/v08-annotated-code.webp)
...
![Pipeline flowchart from mint address through decode-mint (R1) into checkCombo (R2), forking to proceed-on-valid or fix-the-set-on-rejection, gating every later mint build in the course.](assets/v09-flowchart.webp)
...
![One init attempt fans into three outcomes, each proving something different: a client-side refusal, an on-chain rejection, or a success that only the decoded bytes can confirm.](assets/v10-flowchart.webp)
...
![Between your requested extension set and the on-chain bytes sit the CLI, which can refuse or silently add extensions, and the program enforcing only its five rules.](assets/v11-diagram.webp)
