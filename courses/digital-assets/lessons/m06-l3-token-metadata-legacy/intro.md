...
![Timeline comparing release activity in which mpl-token-metadata's JS client stopped at v3.4.0 in February 2025 while mpl-core kept cutting releases through to mid 2026.](assets/v01-timeline.webp)
...
![Diagram of a pNFT transfer where the frozen token account blocks the plain path, so every transfer funnels through Token Metadata's instruction and its Token Auth Rules evaluation.](assets/v02-diagram.webp)
...
![Flowchart deriving the pNFT design, where failed fixes narrow the problem to the asset refusing to move, forcing the freeze-plus-rule-set architecture and leaving open who maintains the list.](assets/v03-flowchart.webp)
...
![Table of all fourteen operations in the flagship rule set's latest revision, five Transfer operations and nine Delegate operations, every single one mapped to the Pass rule.](assets/v04-table.webp)
...
![Chart of the rule set's nine revisions, where zero through six carry fifteen or sixteen real rules, revision seven drops to one, and the latest carries zero.](assets/v05-chart.webp)
...
![Comparison showing the TokenStandard enum, the docs list five variants while the code defines six, with ProgrammableNonFungibleEdition present only in the source.](assets/v06-comparison.webp)
...
![Decision tree where existing Token Metadata and pNFT assets get read and integrated with an enforcement verdict, while new drops take royalties to Core plugins or Bubblegum v2 rulesets.](assets/v07-flowchart.webp)
...
![Byte layout of a RuleSet account where the header points to a tail revision map that points back at nine stacked revisions, each starting with a lib_version byte.](assets/v08-diagram.webp)
