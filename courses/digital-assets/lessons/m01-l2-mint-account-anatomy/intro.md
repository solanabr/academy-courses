...
Because of a decision made years before extensions existed. The classic token program type-checks its accounts by length alone: a mint is 82 bytes, a token account is 165, a multisig is 355. That is the entire test. Length 82, treat the bytes as a mint; length 165, treat them as a token account; 355, a multisig; anything else, reject. Cheap, simple, and completely dependent on those two numbers never colliding.
