...
![Flowchart of a hooked transfer: the client fetches, decodes, and resolves the ExtraAccountMetaList and appends the extras; skipping resolution reverts with a MissingAccount error.](assets/v01-flowchart.webp)
...
![Annotated 35-byte ExtraAccountMeta layout with a one-byte discriminator selecting one of four encodings, 32 bytes of address or packed config, and one byte each for the signer and writable flags.](assets/v02-annotated-code.webp)
...
![The execute instruction's ordered account list, indices 0 through 6, where omitting or reordering one extra shifts every later index and derives the wrong PDA.](assets/v03-diagram.webp)
...
![In the outer transfer the source and destination are writable and the owner signs, but inside Execute all four accounts arrive read-only and non-signer.](assets/v04-diagram.webp)
...
![Four production stances on transfer hooks: Raydium refuses them, pump.fun parks an empty program in the slot, PYUSD leaves the program id null, and Meteora DBC forwards.](assets/v05-comparison.webp)
...
![A four-state spectrum of the transfer-hook slot from absent through null to no-op and armed, with the resolution tax starting the moment a program id lands.](assets/v06-diagram.webp)
...
![ERC-777's dynamic dispatch cost integrators nothing up front but allowed the imBTC reentrancy drain, while Token-2022 charges a resolution tax for a read-only, non-signer hook frame.](assets/v07-comparison.webp)
...
![The naive four-account TransferChecked reverts, while the resolved version appends the hook's extras, the hook program, and the validation account with identical instruction data.](assets/v08-comparison.webp)
