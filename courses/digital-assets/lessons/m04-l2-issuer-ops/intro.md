...
![A confidential transfer produces amount ciphertexts under the sender, receiver, and auditor keys, so the auditor can decrypt every transfer while the public reads only ciphertext.](assets/v01-diagram.webp)
...
![Flowchart contrasting the per-account owner-signature path with the registry path, where one registration enables signature-free provisioning via ConfigureAccountWithRegistry, both converging on manual approval.](assets/v02-flowchart.webp)
...
![Timeline of a confidential transfer where several transactions create context accounts and verify each proof before the transfer executes and contexts close to reclaim rent.](assets/v03-timeline.webp)
...
![Two mint diagrams showing the main confidential SPROUT with a configured auditor and the solo-branch mint adding ConfidentialMintBurn's four supply fields, annotated with combo rules and the PermanentDelegate warning.](assets/v04-diagram.webp)
...
![Dated timeline of the three ZK ElGamal feature gates showing the January 2025 activation, June 2025 security pause, and June 2026 re-enablement, with a recent probe confirming the verifier live.](assets/v05-timeline.webp)
...
![Comparison showing spl-token-cli 5.6.1 covers the holder flow (configure, deposit, apply, withdraw, confidential transfer) while approve, registry provisioning, and all ConfidentialMintBurn operations exist only as raw instructions.](assets/v06-comparison.webp)
...
![Annotated expected output of the verify script: extension present, auditor pubkey matching the generated key, auto-approve false, and an optional fourth line for the ConfidentialMintBurn branch.](assets/v07-annotated-code.webp)
...
![Decision flowchart running the three ordered rejection checks with their exact error slugs before a valid config falls through to the returned object.](assets/v08-flowchart.webp)
