...
Two budget lines fall straight out of that picture. First, context accounts are rent you front and reclaim, per proof, per transfer; at fleet scale that churn is a real line item and your ops runbook should treat closing contexts as mandatory hygiene, not cleanup. Second, freshness: the single-transaction future is real but not here. This is the same `enable_tx_v1` gate you probed last lesson, shipped in Agave 4.2 with an envelope big enough to carry proofs inline, still with no account behind it on mainnet or devnet as of 2026-08-22. Build for the multi-transaction flow today, and re-probe that gate the week you go to production.
