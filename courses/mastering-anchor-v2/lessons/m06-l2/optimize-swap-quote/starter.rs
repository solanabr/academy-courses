...
/// The optimized version must do TWO things the version below does not:
///   1. apply the trading fee (`fee_bps`, in basis points) before quoting, and
///   2. compute the intermediate products in `u128` so large reserves cannot
///      silently overflow a `u64` multiply.
///
/// Right now it ignores the fee entirely and multiplies in `u64`, so its
/// quotes are wrong whenever `fee_bps > 0`. Fix it.
///
