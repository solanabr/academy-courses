...
This is the one bug that survives a careful-looking port, isolated so you can kill it cleanly. A migration swept a v1 file for hardcoded eights — right instinct, because V2 has no magic number — and deleted every one it found. One of them was not a magic number. It was the `.checked_add(8)` on the end of a sizing helper's arithmetic, the link that put the discriminator back, and `INIT_SPACE` does not put it back for you. Everything the helper sizes now comes out 8 bytes short.
