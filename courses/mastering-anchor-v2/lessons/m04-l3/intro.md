...
> Freshness note: this is written against the Anchor V2 release candidate on the 2.x line (the docs tree published under `v2`), 2026-08-22, newest tag `2.0.0-rc.1` (published to crates.io 2026-08-12). `avm` cannot install it, as m01-l2 showed: there is no GitHub Release object for the v2 tag, so there is nothing to attest. Build the CLI from the documented channel instead, `cargo install --git https://github.com/otter-sec/anchor.git --branch anchor-next anchor-cli --locked --force`, and re-check for a newer rc or a stable tag before you build. The machine-default `anchor-cli 1.1.2` is the V1 line and will not compile the `unsafe(dup)`, `CpiHandle`, or Pod-`Account` features below.
