...
1. **Generate the expansion.** From your framework quarter-vault crate, run the two commands from the top of the lesson. If `cargo expand` errors on a macro, make sure you are on the V2 toolchain, not the machine-default `anchor-cli 1.1.2`, which is the V1 line and will not expand `CpiHandle` or Pod-`Account` code. Install the RC the way m01-l2 showed: `avm` cannot fetch it, because the v2 tag has no GitHub Release object for `avm` to attest, so the documented channel is a direct git build from the branch, `cargo install --git https://github.com/otter-sec/anchor.git --branch anchor-next anchor-cli --locked --force` (`2.0.0-rc.1` as of 2026-08-22; re-check for a newer rc or a stable tag before you build).
