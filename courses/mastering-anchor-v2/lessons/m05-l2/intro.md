...
Look at the numerator: `amount_in_with_fee * reserve_out`. Both of those derive from `u64` values. Multiply two numbers near the top of `u64` and the product needs up to 128 bits. Do it in `u64` and the program panics on a large trade against a deep pool, which is exactly the trade you least want to fail on. The fix is not to cap your reserves but to promote to `u128`, multiply there, divide back down, and cast the result to `u64` only once you know it fits (and it always fits, because the output can never exceed `reserve_out`, which is already a `u64`).
...
This is where the economics earns one sentence, and only one, because this is an Anchor lesson and not a markets one. The reason a pool can quote itself with no oracle and no operator is that the curve turns liquidity into a price function: depth becomes stability, and every trade pays the pool for the privilege of moving it. That is a genuinely elegant piece of mechanism design, and it is why the same two-line invariant shows up under Uniswap, under a pump.fun bonding curve, and under the toy you are building right now. You are not inventing it, only wiring it into Anchor.
...
V2 kills the class. A `CpiHandle` holds a Rust borrow of the typed account wrapper. While that handle is live, the borrow checker will not let you touch the typed account, so `reserve_ticket.amount()` during an in-flight handle from `reserve_ticket` is not a runtime surprise but a compile error. The fix is structural rather than a `.reload()`: read every reserve you need *before* you open a handle, which is exactly why step 1 in the handler reads both balances up top, before any `cpi_handle_mut()` exists. There is no mid-CPI read to get wrong, because the language removed the ability to write one.
