...
### One more interface, named and deferred: declare_program!

There is a second way programs talk to a V2 program that belongs in your head even though you will not build it here, so that it is not invisible to you the day you need it. In v1, a program could consume *another* program's IDL at compile time through `declare_program!`, generating a CPI interface from a vendored IDL. It is how one Anchor program calls another by its published interface instead of by matching source versions.

Its V2 story is still evolving and, as of this writing, unverified on the RC. I am not going to pretend a status I have not confirmed. What I will do is tell you where it gets used in anger: the DeFi and RWA Engineering course consumes live protocols' IDLs directly, in its reading-a-live-protocol lesson, and it treats exactly this question as load-bearing. If `declare_program!` does not yet work on the V2 RC when you get there, that course names the fallback rather than pretending: Codama-generated Rust interface crates, or vendored CPI stubs, so cross-program consumption ships either way. Cross-program calling is a whole interface of its own, and that lesson is where it lives.
