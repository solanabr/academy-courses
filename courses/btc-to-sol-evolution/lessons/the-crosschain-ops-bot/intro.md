...
Now run the cold start. `up` holds the terminal by design, since there is no daemon mode here, so background it, which also gives you the shell back for the kill you are about to do.
