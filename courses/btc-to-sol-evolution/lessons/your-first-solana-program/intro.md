...
One line there is easy to mistype in a way that costs you the next command. `sendAndConfirm` returns `Promise<void>`: it reports success by returning and failure by throwing, and it never hands back a signature. The receipt is already in your hands, because a signed transaction carries its own signature — `getSignatureFromTransaction(signed)` reads it out with no extra network call. Assign the result of `sendAndConfirm` to a variable and print that instead, and the line below reads `invoked: undefined`, with nothing to paste into the confirm command.
