...
![The eight course artifacts shown feeding forward module by module and all converging on the crosschain-ops-bot, with ledger.py struck through as the replaced foil.](assets/v01-diagram.webp)
...
![A table of Bitcoin, EVM, and Solana showing what each generation deleted and the price each paid, throughput, gas plus sequential execution, and complexity plus heavy hardware.](assets/v02-table.webp)
...
![Devnet versus mainnet across money, adversaries, mistakes, and the meaning of a green test suite, free and forgiving on the left, real and permanent on the right.](assets/v03-comparison.webp)
...
![A timeline from Neodyme's 2021 pitfalls guide to the April 2026 Drift exploit and the Solana Incident Response Network founded days later in response.](assets/v04-timeline.webp)
...
![Source builds to fingerprint A, the on-chain program yields fingerprint B, and matching the two proves the deployed code matches the published source.](assets/v05-diagram.webp)
...
![The solana-verify verify-from-repo command annotated argument by argument, network URL, program id, GitHub repo, and commit hash, ending in a match-or-mismatch report.](assets/v06-annotated-code.webp)
...
![A table of the three upgrade-authority choices, single key, multisig, and burned-immutable, each with what it buys you and what it costs.](assets/v08-table.webp)
...
![A table mapping each need, verifiable build, exploit learning, pitfalls, audits, audit contents, production code, and closed-source study, to a specific named resource and how to use it.](assets/v09-table.webp)

One more map before the route: the road out also runs through this catalog, and three of its courses are cut to fit what you just built. If what you want next is engineering rigor, strict TypeScript, Rust taken properly up through traits and `Result`, Docker and CI, the whole polyglot ops substrate your bots have been running on instinct, that is the rust-ts-fundamentals course. If it's the token surface, SPL tokens, Token-2022 and its extensions, NFTs, that is digital-assets, which picks up exactly where this course's lamports-only custody stopped. And mastering-anchor-v2 is the deep Anchor course, with a bar worth stating plainly: it assumes you already ship Anchor programs, and one vault lesson here does not make that true, so treat it as the door you take after more Anchor reps, not as your next click. If the ops bot was your favorite build, there's a fourth signpost: its poll-the-chain, persist-a-cursor, resume-idempotently loop is exactly what solana-payments-commerce's module four rebuilds as a production back office, and that's where it grows up.
...
![A flowchart sequencing a devnet program through verifiable build, self-review, external audit, an upgrade-authority decision, protocol integration, and finally mainnet deploy.](assets/v10-flowchart.webp)
