...
![A carteira da Alice e o mint do USDC apontam cada um para uma terceira conta, a conta de token associada dela, que é derivada desse par e guarda o saldo.](assets/v01-diagram.webp)
...
![Um fluxograma mostrando a derivação, um create idempotente incondicional e a transferência agrupados como uma única transação atômica, com a checagem de existência riscada.](assets/v02-flowchart.webp)
...
![Três caminhos convertendo 2.01 em unidades base: o truncamento em float chega uma unidade a menos, o arredondamento aguenta até dois elevado a cinquenta e três, e a interpretação exata da string continua correta.](assets/v03-annotated-code.webp)
...
![Uma tabela de comparação mostrando que o TransferChecked verifica o mint e os decimais on-chain e rejeita divergências enquanto o Transfer simples confia na sua configuração, e que o Transfer está deprecado nas ferramentas atuais.](assets/v04-comparison.webp)
...
![Um endereço de reference novo viaja junto com a transferência como uma conta extra inerte, o livro-razão indexa ele, e uma consulta nesse endereço devolve exatamente um pagamento.](assets/v05-diagram.webp)
...
![Duas linhas do tempo sobre a janela de 150 blocos: reenviar os mesmos bytes mantém uma signature que aterrissa no máximo uma vez, enquanto remontar cria uma segunda signature e cobra do cliente duas vezes.](assets/v06-timeline.webp)
...
![Um pipeline de três fases: a message mutável ganha um fee payer, um tempo de vida por blockhash e instruções, depois o ato de assinar congela os bytes, e a transação imutável é enviada e confirmada.](assets/v07-flowchart.webp)
...
![A verificação flui da reference key pela consulta de signature e pela busca da transação até um delta de saldo em unidades base, com saídas de falha para uma signature ausente, uma transação que falhou ou um valor divergente.](assets/v08-flowchart.webp)
...
![Barras empilhadas comparam um cliente de primeira viagem pagando a taxa base de 5000 lamports mais o depósito único de rent da ATA, mais ou menos um milésimo de SOL, contra um cliente recorrente pagando só essa taxa fixa.](assets/v09-chart.webp)
