...
![Side-by-side of the two Solana Pay URL forms: the wallet-built transfer request exposing every parameter, versus the server-built transaction request carrying only a link and an order id.](assets/v01-comparison.webp)
...
![Flowchart from a stored cart through the QR, the wallet's GET for label and icon, the POST of the account, server-side pricing, signing, and the reference watcher confirming.](assets/v02-flowchart.webp)
...
![Diagram of the returned transaction: a v0 envelope, the customer as unsigned fee payer, a memo with the order id, and a TransferChecked instruction carrying a readonly reference account.](assets/v03-diagram.webp)
...
![The two code constructions for the completion TODOs: appending the reference as a readonly non-signer account, and building a memo instruction whose data is the utf8 order string.](assets/v04-annotated-code.webp)
...
![Three-zone trust diagram: the merchant server trusted only to build, the wallet enforcing that every unsigned required signer is the submitted account, and the customer relying on that check.](assets/v05-diagram.webp)
...
![Timeline from the 2022 Solana Pay spec, through the checkout library becoming a subpackage, to the 2026 repo leading with an agentic payments CLI.](assets/v06-timeline.webp)
...
![Workspace diagram, transfer-kit and checkout feeding checkout-txreq with its catalog, builder, server, and smoke test, which the POS stall, the drop blink, and the capstone consume in later lessons.](assets/v07-diagram.webp)
