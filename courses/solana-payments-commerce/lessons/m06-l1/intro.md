...
- **The way out** is Coinbase Offramp, and it is hosted-only: you redirect the user into Coinbase's own flow rather than embedding it. Payout runs over ACH or PayPal.
...
5. **The funded session, guided.** This step needs CDP keys and your app's trial-mode clearance, and so does the offramp walk in step 6, so bank the pair together for when you have credentials; if you are offline or keyless today, the smoke test alone completes the lesson's build, and steps 5 and 6 keep. Export `CDP_API_KEY_ID` and `CDP_API_KEY_SECRET`, start the route with `npx tsx ramp-embed/server.ts`, then play the storefront client from a second terminal:
   ...
   The JSON that comes back holds a live onramp URL, and the raw token is sitting in it as the `sessionToken` query param. Remember what the theory said about its lifetimes: single-use, five minutes. Each curl mints one token good for one attempt, so re-run the curl whenever you need another; the sandbox below and step 6 each get their own fresh mint. Open the URL in a browser and walk it as the buyer: the widget opens on USDC on Solana with 12.5 dollars pre-filled, and the Apple Pay or card sheet Coinbase shows is the exact surface a Wavelength buyer would see. Whether you complete the small purchase is your call and your trial limit.

   If you would rather not point a real card at a real wallet today, Coinbase runs a sandbox for the guest-checkout card flow: mint a fresh token with the same curl and hand it to `https://pay-sandbox.coinbase.com/?sessionToken=<token>` with the same display presets (verified against the guest-checkout sandbox docs at write; if the sandbox host rejects your token, mint again and re-check the docs' sandbox section, the part of this surface most likely to drift). The sandbox card form takes `4242 4242 4242 4242` for a success and any other sixteen digits for a decline, and any six-digit code passes the phone step. No real funds move. Either host is a legitimate completion of this step, because the integration point being graded is the same in both: the URL you opened came out of your server route, bound before the browser ever saw it.

6. **The offramp walk, guided.** No new code, deliberately: the way out is hosted, so the build is a redirect you can assemble by hand and the learning is in walking it. Like step 5, this needs your CDP keys. Mint a fresh session token the same way (same endpoint; step 5's server route already does it, and for a sell session the bound address is the wallet the USDC leaves from rather than a destination), then form the hosted URL: `https://pay.coinbase.com/v3/sell/input` with your `sessionToken`, a `partnerUserRef` naming the artist in your books (any opaque string under 50 characters, never their real identity), and a `redirectUrl` you control. One prerequisite hides in that last param: redirect domains are allowlisted in your CDP dashboard's Onramp settings before Coinbase will accept them, so register `http://localhost:3200` there first, and when the hosted flow refuses to open, a rejected, unregistered redirect is the first thing to check. Open it and narrate what you see against the theory: the sign-in is the artist's Coinbase relationship, the identity checks are theirs and not yours, the USDC send is from their wallet, and the payout options ending in a bank are the ACH and PayPal rails from the theory. Then close the loop out loud, because this is the assessment's muscle: say who is merchant-of-record for the embedded onramp, for this hosted offramp, and for a Stripe-onramp card purchase, one line each. If any of the three takes you more than a sentence, reread the seam map before moving on.
