...
So why does the float argument survive that concession? Look at which rail PIX actually beat. Brazilian card volume splits R$3.1 trillion on credit against R$1 trillion on debit, and 42.6% of that credit volume is parcelado. PIX won the debit fight: the instant pay-now sale already belongs to it. What it did not replace is credit, because a credit sale is not a transfer that happens to be slow; it is the customer buying delay itself, the right to pay later and in pieces, and PIX does not sell delay. The float, and the roughly 1.05% a month it costs to escape it, is a credit cost, funded by the merchant, riding on the one rail PIX left standing. That is the comparison these rails actually face in Brazil: not PIX, whose fight is over, but the credit stack, whose full price you now know how to read: the visible percentage, plus the float, on terms that get worse as you get smaller. Cheap rails do not just make existing payments cheaper. They change which of a merchant's costs are laws of nature and which are line items you can refuse.
