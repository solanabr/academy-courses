...
![A translation table pairing authorization with processed, capture with confirmed, and settlement with finalized, footnoted that card settlement is reversible by chargeback while finalized is not.](assets/v01-comparison.webp)
...
![A payment moves from submitted to processed in one 300ms slot, to confirmed in one to two seconds, then to irreversible finalized in about nine seconds.](assets/v02-flowchart.webp)
...
![A log-scale bar chart of card fees climbing from 31 cents to 58 dollars across four ticket sizes, while the Solana base fee stays flat and sub-cent.](assets/v03-chart.webp)
...
![A grouped bar chart of InfinitePay's published top-tier rates: credit à vista at 1.62 percent on the standard schedule against 2.69 percent at D+1, a 12x sale at 2.25 percent per installment against 8.99 percent at D+1, and a takeaway band pricing both spreads at roughly 1.05 percent a month.](assets/v04-chart.webp)
...
![Card rails pull funds with a dashed chargeback arrow running backward for weeks; Solana pushes funds with no backward arrow, so a refund is a separate new payment.](assets/v05-diagram.webp)
...
![A two-column matrix: strong fit for small-ticket sales, flows that need to escape settlement float, and, last, cross-border payments; weak fit for high-fraud sales, businesses selling reversibility, and compliance regimes requiring an institutional undo.](assets/v06-comparison.webp)
...
![Parallel timelines of a 2,000-dollar sale: Solana reaches irreversible finality in about nine seconds, while card rails settle in days and leave the chargeback window open for weeks.](assets/v07-timeline.webp)
