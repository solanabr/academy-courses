...
**Attack 5: a real payment for a different order.** Subtlest of the five. The transaction is genuine, right program, right mint, right amount, paid to you. It is just order `ord-0999`'s payment, and the buyer is replaying its signature against order `ord-1024`. Amount checks cannot catch this when two records cost the same. The order id your transaction-request builder stamps into the spl-memo is the binding: the verifier finds the parsed `spl-memo` instruction and requires the memo string to contain the expected order id, returning `wrong-reference` otherwise. The verifier deliberately checks containment, not equality, so the exact memo format stays the builder's business and the two modules cannot drift apart.
...
export function createVerifier(deps: {
  ...
  return async function verify(
    ...
    const memoMatches = memosOf(tx).some((m) => m.includes(expected.orderId));
    if (!memoMatches) {
