...
One reality check before you continue, because the program's newest version is this fresh: the devnet deployment can lag the mainnet one. Probe it first, `solana program show De1egAFMkMWZSN5rYXRj9CAdheBamobVNubTsi9avR44 --url devnet` must print an executable program (it did on 2026-08-22). Version skew only reveals itself at the first instruction: if this call or any later one fails with custom program error 133 or 134 (the client names them `DELEGATION_VERSION_MISMATCH` and `MIGRATION_REQUIRED`), the devnet binary predates your 0.5.0 client, and you have two real options. If you carry a Rust toolchain: clone the program repo at its `program-v0.5.0` tag, `cargo build-sbf`, run `solana-test-validator --reset`, deploy the built binary locally, and point `RPC_URL` at `http://127.0.0.1:8899` for the rest of the lab; call it a twenty-minute detour. If you do not: drop this one workspace to the documented fallback pin from the seam section, `@solana/subscriptions@0.4.0` with kit `^6.4`, and continue against the older devnet binary. Check first, rage later.
