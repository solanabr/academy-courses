...
npm error notsupported Required: {"node":">=99"}
npm error notsupported Actual:   {"npm":"11.6.0","node":"v24.7.0"}
```

Read that like a working dev instead of pasting it into a search box. `Required` is what your package.json claims. `Actual` is the machine it landed on. The install refused because a written promise and a real environment disagreed. That is not an error message, it is a contract clause firing, and every field we cover today fires exactly like this on somebody's machine, eventually. Revert the sabotage and reinstall before moving on.

## Summary

The findings up front:

- `package.json` is a contract with machines and with consumers you will never meet. `exports` promises an API surface, `engines` promises a runtime range, `packageManager` pins the tool, dependency ranges promise which futures you accept, and `peerDependencies` promises what you expect the CONSUMER to provide.
- You will extract the fleet's engine (the `ProbeResult` union, `classifyProbe`, the backoff helpers) into `pulse-core`, a pnpm workspace package. The m02-l4 test suite must be green before and after: structure changes, behavior does not.
...
- The live peer artifact: `helius-sdk` 3.1.0 peers `@solana/kit ^6.9.0` while kit's latest sits at 8.2.0, two majors ahead (both probed 2026-09-02). Install them together and npm refuses. The durable rule, planted here and paid off in M8: pin what your deps peer against, per workspace, never a memorized digit.
- The lab's autonomy fades on schedule: I drive the first extraction move diff by diff, you move the remaining modules from the same recipe, and the peer-conflict diagnosis plus the coding challenge are yours alone.

## The contract, clause by clause

Here is the collapse that makes the whole file make sense: a package is a promise about environments you'll never see. Your code will run on a laptop you never touched, a Node version you never installed, next to dependency versions you never chose, imported by a person you will never meet. `package.json` is where you write down which of those futures you are promising to survive. Every field below is one clause of that promise, and the opening sabotage already showed you what enforcement looks like when a clause and reality disagree.
...
The honest rule is not "always extract". A package boundary costs ceremony: two `package.json` files to keep truthful, an exports map to maintain, and every internal refactor now asks "is this API public?" Copy-paste has none of those costs, right up until the second consumer appears. So: extract when the second consumer is REAL, not speculative. Ours is scheduled. Next lesson the dashboard renders `status.json` using the same classifier the fleet publishes with, and if those two disagree about what "degraded" means, the board lies to human eyes. M7's edge worker makes three consumers. The extraction happens now because the drift window opens now.
...
Fair question before we commit: npm has workspaces too, so why is this lesson a pnpm lesson? Two honest reasons. First, the ecosystem chose: every serious Solana TypeScript repo you will read is a pnpm workspace, and reading the wild fluently is a stated goal of this course. Second, pnpm's stricter dependency layout (packages only see what they declare, instead of whatever happens to be hoisted within reach) means a missing dependency line fails on your machine today instead of on a consumer's machine after publish, and for a package headed to npm in three lessons that strictness is a feature we want pointed at ourselves.
...
`engines` declares which runtimes you claim to support. The verb matters: claim. Nothing tests your code on Node 20 because you typed `>=20`. Out in the wild there is a visible gap between what people run and what libraries promise: the ecosystem runs Node 24 (the current LTS line as of this writing, 2026-09-02, with the Node 26 LTS handover scheduled for 2026-10-28), while library floors observed in survey sit at `>=20` or `^22`, because maintainers keep supporting runtimes their users have not left yet. `@solana/kit` itself declares `engines.node >=20.18.0` while its maintainers certainly develop on newer.

Why do the floors trail the runtime by two LTS lines? Because a library's engines field is not a fashion statement, it is the intersection of every environment its users still deploy to. A team stuck on a Node 20 container base image reads `>=20` as "this package will not strand us", and a maintainer who bumps the floor to `>=24` on a whim just cut every one of those teams off from bug fixes for no technical reason. The floor moves when the code actually needs a newer API, or when the old line leaves LTS entirely, and not before. Reading an engines field therefore tells you something real about a package's audience: kit's `>=20.18.0` says "we still carry the fleet that has not migrated", and that is a deliberate, costly kindness.
...
Now the install-path honesty beat, because it changed recently and most tutorials have not caught up. The tool that historically read this field and auto-activated the right pnpm was corepack, shipped inside Node. On 2025-03-19 Node's Technical Steering Committee voted corepack out: it is no longer distributed with Node 25 and later (the docs PR making the removal official merged 2026-01-01), and it remains only in Node 24 LTS. The runtime decided package managers are not its job. So the durable install line is the boring one:
...
Notice which of these you actually author. Almost none: when you run `npm i some-dep` or `pnpm add some-dep`, the tool writes a caret range into your `package.json` for you, which means most dependency lines in most repos are a policy their author never consciously chose. That is worth sitting with for a second. The default is defensible (fixes flow, majors blocked), but it is still a default, and teams that want exact pins flip it deliberately with `npm config set save-exact true` and lean on the lockfile plus an update tool instead of ranges. Either stance is coherent; drifting into one because a tool wrote it for you is the only wrong option.
...
Why refuse ownership at all? Suppose helius-sdk declared kit as a regular dependency instead. Your app installs its own kit, helius-sdk installs a second private one, and now two copies of the same library live in one process. Every value that crosses between them (an rpc object your code builds, an address type the sdk returns) was constructed by one copy and inspected by the other, and identity-based checks fall apart in ways that produce the least helpful error messages in the ecosystem: the object IS a valid thing, just a valid thing from the wrong copy. The peer declaration is how a library says "we must share the one instance", and the range on it is how it says which instances it has actually been tested sharing.
...
Meanwhile `@solana/kit`'s latest sits at 8.2.0, two majors ahead of that `^6.9.0` peer range. Install `helius-sdk` and then ask for kit latest in the same package, and npm's resolver refuses with an `ERESOLVE` error. In the lab you will trigger this on purpose and read the output slowly, because the refusal is the resolver protecting something real: helius-sdk's code calls kit APIs from the 6.x line it was built and tested against. Forcing kit 8 next to it does not upgrade helius-sdk, it just runs a library against an API surface it has never seen, and the failures move from install time (loud, immediate) to runtime (quiet, in production).
...
Why a rule instead of a number? Because the digits rot on a timescale you would not believe if it were not in npm's own timestamps: kit shipped two majors in just over nine weeks, hot on a minor's heels. 6.10.0 on 2026-06-16, then 7.0.0 on 2026-06-30, then 8.0.0 on 2026-08-21. Any lesson, blog post, or team wiki that froze "use kit 6" was wrong twice before the season changed. The peer ranges in your actual `node_modules` are the only version advice that updates itself. M8 lesson two builds the Solana workspace where this rule stops being a drill and starts being the setup step.
...
The skill this lesson is actually installing is manifest literacy, so close the theory by reading a real one, no lab steps, just eyes. Pull up any serious Solana TypeScript repo (kit itself is the one this course keeps citing) and read its `package.json` top to bottom asking one question per field: what is this line promising, and to whom? You will find the `packageManager` pin (all five repos surveyed for this course carry one). You will find an engines floor older than the Node on your machine, and now you know that gap is audience data, not neglect. You will find a preinstall guard in kit's case whose whole job is refusing the wrong package manager. And you will find `exports` maps considerably fancier than ours, with conditional entries per environment; the conditions system is real depth we are deliberately not teaching today, and it lives on the bookmarked path below. Two minutes of this per unfamiliar repo, every time, and the wild stops being intimidating: the repo tells you its rules before you run a single command in it.
...
2. **Declare the workspace and move the fleet into it.** From the repo root, create the layout and relocate everything fleet-shaped into `packages/pulse-fleet` (your filenames may differ; move what you have):
   ...
   git mv src tests package.json tsconfig.json vitest.config.ts pulse.config.json packages/pulse-fleet/
   ```

   Note `tests` in that list: the m02-l4 suite imports `../src/config.js` and reads `./fixtures/`, so it must stay a sibling of `src/` or the checkpoint below runs zero tests. Then open the moved `packages/pulse-fleet/package.json` and set its `"name"` field to `"pulse-fleet"`. The file arrived still carrying the old root name, the root manifest below is about to reuse that name for the private glue, and pnpm keys everything on the name field, not the directory: `pnpm -r` output prefixes, `--filter` selectors (step 7 depends on one), and m03-l3's Vercel build-skip all need a unique name per package.
   ...
   Your `.github/workflows` directory stays at the root, because Actions only reads workflows from there. But notice what the move just did to the pipeline: every job in `pulse.yml` still runs `npm ci` against a root whose `package.json` is now private glue with no lockfile. Push right now and all three jobs go red and the cron stalls. That is expected, and step 7 rewires it before anything gets pushed. While you are here, delete the stale `package-lock.json` at the root: pnpm writes its own `pnpm-lock.yaml` on the next install, and that file (commit it) is the workspace's real pin from now on. Checkpoint: `pnpm install` from the root completes and `pnpm -r test` runs the m02-l4 suite green from its new home. Nothing is extracted yet; we just proved the move broke nothing before changing anything else.

3. **Create pulse-core and move the first engine module.** The classifier and its union go first, because they are the module the next lesson imports. Mine live in `src/probe.ts` (the `ProbeResult` union plus `parseProbe`) and `src/classify.ts` (`classifyProbe` plus `assertNever`); if yours are named differently the moves are identical:

   ```bash
   git mv packages/pulse-fleet/src/probe.ts packages/pulse-core/src/
   ...
   export { parseProbe } from "./probe";
   export { classifyProbe, assertNever } from "./classify";
   ```
...
4. **Wire the fleet to import across the boundary.** In `packages/pulse-fleet/package.json`, add the workspace dependency:
   ...
   Then run `pnpm install` from the root to create the symlink, and update every fleet import of the moved modules:

   ```ts
   // before
   import { classifyProbe } from "./classify";
   import { parseProbe, type ProbeResult } from "./probe";

   // after
   import { classifyProbe, parseProbe, type ProbeResult } from "pulse-core";
   ```

   Your test files change the same one line and nothing else. That is the point worth pausing on: the m02-l4 suite now exercises `pulse-core` through its public boundary, exactly the way next lesson's dashboard will. Checkpoint: `pnpm -r test` green again, and the output itself teaches you how `-r` thinks. It walks every workspace package and runs the `test` script in each one that has it, prefixing output with the package name, something shaped like:
   ...
   Your counts will match whatever your m02-l4 suite grew to; the shape is what to recognize. Only `pulse-fleet` runs tests right now because only it has a `test` script, and that is fine today: the suite crosses the boundary, so the core is exercised. When `pulse-core` grows its own script, `-r` picks it up with zero config, which is most of what "monorepo-lite" means in practice. If TypeScript cannot resolve `pulse-core`, you skipped the root `pnpm install` that creates the link; if it resolves but complains about the entry point, your `exports` path does not match where `index.ts` actually sits.
...
6. **Prove the boundary forbids the back door.** From any fleet file, try the deep import the exports map exists to kill, `import { classifyProbe } from "pulse-core/src/classify"`, and run the tests. Read the `ERR_PACKAGE_PATH_NOT_EXPORTED` failure, appreciate it briefly, delete the line. Acceptance for the extraction: `pnpm -r test` green from the root, and `grep -r "classifyProbe" packages/pulse-fleet/src` shows only import lines, zero function bodies. Engine code lives in exactly one place.

7. **Re-wire the pipeline (the workspace's third consumer).** Step 2's warning comes due: `pulse.yml`'s jobs still install with `npm ci` and run their tools from a root that no longer holds the fleet. Teach the workflow the layout you just taught yourself. In all three jobs, the install line becomes a pair:
   ...
   Then point each job's tool at the workspace: the typecheck job's gate becomes `- run: pnpm --filter pulse-fleet exec tsc --noEmit`, the test job's becomes `- run: pnpm -r test` (the exact command you have been running locally all lab), and the probe job's run steps gain `working-directory: packages/pulse-fleet` so the fleet's tools run from its new home, with one contract deliberately unchanged: `status.json` stays at the REPO ROOT. That path is load-bearing, because m03-l2's dashboard and m03-l3's config both fetch the raw file at `.../main/status.json`, so point the fleet's write at the root (write to `../../status.json`, or take the output path as an argument) and keep the commit step adding `status.json` from the repo root. One file, one canonical path; if a second copy ever appears inside `packages/pulse-fleet/`, that is the write aimed wrong, and the board would silently render whichever copy the root holds, frozen. `--frozen-lockfile` is CI's honoring install: take exactly what `pnpm-lock.yaml` recorded or fail loudly, which is `npm ci`'s job in pnpm spelling. Commit the workflow change together with `pnpm-lock.yaml`, push, and watch the run. Checkpoint: both gates green, the probe job commits a fresh `status.json`, and the `needs: [typecheck, test]` edges from m02-l4 survive untouched. The pipeline turned out to be the extraction's third consumer, and because `-r` walks whatever the workspace declares, every future package you add is already inside the gate.
...
8. **The diagnosis rep (unguided).** In a scratch directory, reproduce the theory section's conflict with your own hands and read the refusal:
   ...
   The first install succeeds (npm auto-installs the declared peers, all from the 6.x-compatible lines). The second refuses with `ERESOLVE`, and buried in the block is the sentence that matters, shaped like `peer @solana/kit@"^6.9.0" from helius-sdk@3.1.0`. Your deliverable is ONE written sentence diagnosing the fix in pin-rule terms. It must name the range and the rule; if your sentence contains a bare version digit as the answer, it will be stale before the module ends, and you have re-read the theory section's timeline chart to see why.

## Challenge

The semver logic you just used by eye becomes code you write: implement `satisfiesRange(version, range)` for the four range forms this lesson taught, exact, `>=`, tilde, and caret, including the caret-zero rule where major 0 makes the minor the breaking slot. `parseSemver` and `compare` are provided in the starter, which lives where every challenge in this course does: the interactive coding-challenge panel on this lesson's page; the exact-match arm is done for you. Eleven tests grade it, and one of them is this lesson's lab in miniature: does `7.0.2` satisfy `^6.9.0`? Same rule your `ERESOLVE` block enforced with 8.2.0, and your implementation should agree with npm's resolver: it does not. Hints live in the starter file, escalating from operator-ordering to the caret-zero branch, so spend them in order and only as needed.

One design note before you start, because it is the first hint in disguise: two of the operators are one character and one is two, so the ORDER you test them in is load-bearing. Check for `>=` before you check for anything single-character, or the parser half of your brain will slice the wrong prefix and every floor test fails at once, which looks like a logic bug and is actually a string bug. After this challenge, a caret in any manifest you ever read is something you can compute, not something you squint at, and that is the difference between reading a peer conflict and being read to by one.

## Checkpoint, and the first outside consumer

What you can now do, concretely: read any `package.json` in the wild and say what each field promises and to whom; run a two-package pnpm workspace where structure changed and a green test suite proves behavior did not; and diagnose a peer conflict by reading ranges instead of force-flagging past them. The 30-second retrieval before you close the tab, say it out loud: what does `^6.9.0` admit, and where does it stop? (Up through any 6.x at or above 6.9.0. Never 7. And if that base had been `^0.9.0`, the stop moves to the minor.)
