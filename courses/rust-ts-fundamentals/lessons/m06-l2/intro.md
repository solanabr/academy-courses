...
Enter the username and the token or password it asks for; `Login Succeeded` is your checkpoint. (The free Personal plan also carries one private image repository, which is more hosting than this course will ask of it.) That is the whole fix. When you want your own images hosted next to your code rather than on Docker's registry, GitHub's registry GHCR is the escape hatch, and it is exactly where we push the poller's image in m06-l4. Not today.
