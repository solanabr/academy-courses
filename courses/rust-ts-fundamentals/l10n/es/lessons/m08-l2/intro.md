...
Un cuadro sobre el futuro, para que los docs no te embosquen. La guía de upgrade del sitio de kit ahora abre con una API estilo plugin: `createClient()` con composición `.use(...)`. Es trabajo propio de kit, es hacia donde va la librería, y el panel no la usa. Los paquetes de plugin que la respaldan estaban en 0.19.0 en mi sonda al momento de escribir, y pasaste una lección entera sobre qué quiere decir pre-1.0: las minors cargan derechos de cambio de ruptura. El estilo pipe-y-RPC que enseña esta lección es el camino estable documentado, el dialecto que hablan hoy los ejemplos propios del ecosistema de kit y sus clientes generados, así que todo lo que cableaste acá es cimiento, no un callejón sin salida. Cuando los plugins crucen 1.0, vuelve a correr este cálculo; el orden de la página de docs es marketing, las versiones de los paquetes son evidencia.
